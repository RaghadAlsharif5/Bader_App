...
class AuthCubit extends Cubit<AuthStates> {
  ...
  void register(
    ...
    final result =
        await sl<RegisterUseCase>().execute(email: email, password: password);
    ...
    result.fold((serverFailure) {
      ...
      UserModel userModel = UserModel(
          ...
          null,
          null);
      await SendUserDataToFirestoreUseCase(
              authBaseRepository: sl<AuthImplyRepository>())
          .execute(user: userModel, userID: right.user!.uid);
  ...
  void login(
      {required String email,
      required String password,
      required LayoutCubit layoutCubit}) async {
    emit(LoginLoadingState());
    final result =
        await sl<LoginUseCase>().execute(email: email, password: password);
    debugPrint("Result is : $result");
    // TODO: left => result that on the left side ( Instance from Failure ) , right => Instance from UserCredential
    result.fold(
        (serverFailure) =>
            emit(LoginStateFailed(message: serverFailure.errorMessage)),
        (userCredential) async {
      bool appOnRealDevice = await Constants.isPhysicalDevice();
      debugPrint(
          "App run on real Device is : $appOnRealDevice , operating id is : ${Platform.operatingSystemVersion}");
      if (appOnRealDevice == true) {
        await FirebaseMessaging.instance.subscribeToTopic(
            "all"); // TODO: عشان حتى الانين لو حبت ابعت حاجه لكل المتسخدمين هبعت من خلال Topic ده
        await sl<UpdateMyFirebaseMessagingTokenUseCase>()
            .execute(userID: userCredential.user!.uid);
      }
      await SharedPref.insertString(
