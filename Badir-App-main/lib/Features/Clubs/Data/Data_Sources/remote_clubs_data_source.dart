...
class RemoteClubsDataSource {
      ...
      if( respondStatus )     
        {
          ...
          idForClubsThatSenderMemberIn.add(clubID); 
          if( committeesNameForRequestSender.contains(committeeNameForRequestSender) == false ) committeesNameForRequestSender.add(committeeNameForRequestSender);
  ...
  Future<Unit> removeMemberFromClubILead({required String memberID,required String clubID}) async {
    try
    {
      ...
      await Future.value(idForClubsThatHeMemberIn.remove(clubID));  
