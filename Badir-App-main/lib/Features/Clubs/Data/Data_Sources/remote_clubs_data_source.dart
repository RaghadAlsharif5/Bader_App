...
class RemoteClubsDataSource {
  Future<String> uploadClubImageToStorage({required File imgFile}) async {
    try {
      ...
      throw ServerException(
          exceptionMessage: "Something went wrong, try again later");
    }
  }
  ...
  Future<Set<String>> getMembersOnMyClub({required String clubID}) async {
    ...
    try {
      await FirebaseFirestore.instance
          .collection(Constants.kClubsCollectionName)
          .doc(clubID)
          .collection(Constants.kMembersDataCollectionName)
          .get()
          .then((value) {
  ...
  Future<bool> requestAMembershipOnSpecificClub(
      {required String clubID,
      String? senderFirebaseFCMToken,
      required String requestUserName,
      required String userAskForMembershipID,
      required String infoAboutAsker,
      required String committeeName}) async {
    try {
      final model = RequestMembershipModel(
          userAskForMembershipID,
          infoAboutAsker,
          committeeName,
          requestUserName,
          senderFirebaseFCMToken);
      await FirebaseFirestore.instance
          .collection(Constants.kClubsCollectionName)
          .doc(clubID)
          .collection(Constants.kMembershipRequestsCollectionName)
          .doc(userAskForMembershipID)
          .set(model.toJson());
      return true;
    } on FirebaseException catch (e) {
      return false;
  ...
  Future<Unit> updateClubData(
      {required String clubID,
      required String image,
      required String name,
      required int memberNum,
      required String aboutClub,
      required ContactMeansForClubModel contactInfo}) async {
    try {
      await FirebaseFirestore.instance
          .collection(Constants.kClubsCollectionName)
          .doc(clubID)
  ...
  Future<Set> getIDForClubsIAskedForMembership(
      {List? idForClubsMemberID, required String userID}) async {
    Set clubsID = {};
    await FirebaseFirestore.instance
        .collection(Constants.kClubsCollectionName)
        .get()
        .then((value) async {
      for (var item in value.docs) {
        if ((idForClubsMemberID != null &&
                idForClubsMemberID.contains(item.id) == false) ||
            idForClubsMemberID == null) {
  ...
  Future<Unit> updateClubAvailability(
      {required String clubID,
      required bool isAvailable,
      required List availableOnlyForThisCollege}) async {
    try {
      await FirebaseFirestore.instance
          .collection(Constants.kClubsCollectionName)
          .doc(clubID)
  ...
  Future<int> getMembersNumOnApp() async {
    int membersNum = 0;
    await FirebaseFirestore.instance
        .collection(Constants.kMembersNumberCollectionName)
        .doc('Number')
        .get()
        .then((value) {
  ...
  Future<int> getNumOfRegisteredMembersOnSpecificClub({required clubID}) async {
    ...
    await FirebaseFirestore.instance
        ...
        .get()
        .then((value) async {
      membersNum = value.data() != null
          ? value.data()!['numOfRegisteredMembers'] ?? 0
          : 0;
    });
    return membersNum;
  }

  Future<void> acceptOrRejectMembershipRequest(
      {required String committeeNameForRequestSender,
    ...
    try {
      ...
      if (respondStatus) // TODO: Request Accepted
      {
        ...
        int numOfRegisteredMembersOnClub =
            await getNumOfRegisteredMembersOnSpecificClub(clubID: clubID);
        await FirebaseFirestore.instance
            .collection(Constants.kClubsCollectionName)
            .doc(clubID)
            .update({'numOfRegisteredMembers': ++numOfRegisteredMembersOnClub});
        // TODO: Get Data about RequestSender....
        DocumentSnapshot requestSenderDocumentSnapshot = await FirebaseFirestore
            .instance
            .collection(Constants.kUsersCollectionName)
            .doc(requestSenderID)
            .get();
        UserModel requestSenderUserModel =
            UserModel.fromJson(json: requestSenderDocumentSnapshot.data());
        List? idForClubsThatSenderMemberIn =
            requestSenderUserModel.idForClubsMemberIn ?? [];
        List? committeesNameForRequestSender =
            requestSenderUserModel.committeesNames ?? [];
        // TODO: بالنسبه لتاريخ بدء العضوية للشخص ده لو في قيمه مش هعدل عليها لو مفيش هبعت التاريخ الحالي
        String memberShipStartDateForRequestSender =
            requestSenderUserModel.membershipStartDate ??
                Constants.getTimeNow();
        // TODO: في key في الداتا بتاع العضو اسمها idForClubMemberIn محتاج ابعت id بتاع النادي لها بما ان اصبح عضو فيه
        idForClubsThatSenderMemberIn.add(
            clubID); // TODO: I will update requestSenderData specially the key ( idForClubsMemberIn )
        if (committeesNameForRequestSender
                .contains(committeeNameForRequestSender) ==
            false)
          committeesNameForRequestSender.add(
              committeeNameForRequestSender); // TODO: I will update requestSenderData specially the key ( idForClubsMemberIn )
        ...
        await FirebaseFirestore.instance
            .collection(Constants.kMembersNumberCollectionName)
            .doc('Number')
            .set({'total': ++membersNum});
        MemberModel member = MemberModel(
            memberName: requestSenderUserModel.name!,
            memberID: requestSenderID,
            membershipDate: Constants.getTimeNow());
  ...
  Future<List<RequestMembershipModel>> getMembershipRequests(
  ...
  Future<List<MeetingModel>> getMeetingRelatedToClubIMemberIn(
      {required List idForClubsMemberIn}) async {
    try {
      ...
      await FirebaseFirestore.instance
        ...
        for (var item in value.docs) {
          if (idForClubsMemberIn.contains(item.id.trim())) {
            await item.reference
                .collection(Constants.kMeetingsCollectionName)
                .get()
                .then((val) async {
  ...
  Future<List<MeetingModel>> getMeetingCreatedByMe(
      {required String clubID}) async {
    try {
      List<MeetingModel> meetings = [];
      // ف كلا الحالتين هحذف م الطلبات
  ...
  Future<Unit> createMeeting(
    ...
    try {
      ...
      QuerySnapshot querySnapshot = await FirebaseFirestore.instance
          ...
          .collection(Constants.kMeetingsCollectionName)
          .get();
      int newMeetingID = querySnapshot.docs.isNotEmpty
          ? int.parse(querySnapshot.docs.last.id)
          : 0;
      ++newMeetingID;
      MeetingModel meetingModel = MeetingModel(name, newMeetingID.toString(),
          description, date, time, location, link);
      await FirebaseFirestore.instance
          .collection(Constants.kClubsCollectionName)
          .doc(idForClubILead)
          .collection(Constants.kMeetingsCollectionName)
          .doc(newMeetingID.toString())
          .set(meetingModel.toJson());
      return unit;
    } on FirebaseException catch (e) {
      throw ServerException(exceptionMessage: e.code);
    }
  ...
  Future<Unit> updateMeeting(
    ...
    try {
      ...
      MeetingModel meetingModel = MeetingModel(
          name, meetingEntity.id, description, date, time, location, link);
      await FirebaseFirestore.instance
          .collection(Constants.kClubsCollectionName)
          .doc(idForClubILead)
          .collection(Constants.kMeetingsCollectionName)
          .doc(meetingEntity.id)
          .set(meetingModel.toJson());
      return unit;
    } on FirebaseException catch (e) {
      throw ServerException(exceptionMessage: e.code);
  ...
  Future<Unit> deleteMeeting(
      {required String meetingID, required String clubID}) async {
    try {
      await FirebaseFirestore.instance
          .collection(Constants.kClubsCollectionName)
          .doc(clubID)
  ...
  Future<Unit> removeMemberFromClubILead(
    ...
    try {
      ...
      await FirebaseFirestore.instance
          ...
          .update({'numOfRegisteredMembers': --numOfRegisteredMembersOnClub});
      // TODO: Get Data about RequestSender....
      DocumentSnapshot memberDocumentSnapshot = await FirebaseFirestore.instance
          .collection(Constants.kUsersCollectionName)
          .doc(memberID)
          .get();
      UserModel memberModel =
          UserModel.fromJson(json: memberDocumentSnapshot.data());
      List? idForClubsThatHeMemberIn = memberModel.idForClubsMemberIn!;
      await Future.value(idForClubsThatHeMemberIn.remove(
          clubID)); // TODO: Delete Club ID from This list, will update his date on Firestore)
      await FirebaseFirestore.instance
          .collection(Constants.kUsersCollectionName)
          .doc(memberID)
          .update({
        'idForClubsMemberIn': idForClubsThatHeMemberIn.isNotEmpty
            ? idForClubsThatHeMemberIn
            : null
      });
