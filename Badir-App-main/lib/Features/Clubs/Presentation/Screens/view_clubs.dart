...
class ViewClubsScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final ClubsCubit clubsCubit = ClubsCubit.getInstance(context)
      ..changeSearchAboutClubStatus(value: false);
    clubsCubit.filteredClubsData.clear();
    // TODO: userID check it's value to know if he visitor or not
    UserEntity? myData = Constants.userID != null
        ? LayoutCubit.getInstance(context).userData!
        : null;
    if (Constants.userID != null &&
        clubsCubit.idForClubsIAskedToJoinAndWaitingResponse.isEmpty)
      clubsCubit.getAllClubs(userEntity: myData);
    return SafeArea(
        ...
        child: BlocBuilder<ClubsCubit, ClubsStates>(
          ...
          return Scaffold(
              appBar: AppBar(
                ...
                title: clubsCubit.searchEnabled
                    ? TextField(
                ...
                actions: [
                  Padding(
                    ...
                    child: GestureDetector(
                      ...
                      child: clubsCubit.searchEnabled
              ...
              body: BlocConsumer<ClubsCubit, ClubsStates>(
                  buildWhen: (pastState, currentState) =>
                      currentState is GetClubsDataSuccessState ||
                      currentState is GetFilteredClubsSuccessState,
                  listener: (context, state) {
                    ...
                    if (state is FailedToSendRequestForMembershipState) {
                      ...
                      showToastMessage(
                          context: context,
                          message: "حدث خطأ اثناء ارسال الطلب",
                          backgroundColor: Colors.red);
                    }
                  },
                  builder: (context, state) {
                    if (state is GetClubsDataSuccessState ||
                        state is GetFilteredClubsSuccessState ||
                        clubsCubit.clubs.isNotEmpty) {
                      return ListView.builder(
                        itemCount: clubsCubit.filteredClubsData.isEmpty
                            ? clubsCubit.clubs.length
                            : clubsCubit.filteredClubsData.length,
                        itemBuilder: (context, index) {
                          return _clubItem(
                              myData: myData,
                              club: clubsCubit.filteredClubsData.isEmpty
                                  ? clubsCubit.clubs[index]
                                  : clubsCubit.filteredClubsData[index],
                              context: context,
                              cubit: clubsCubit,
                              requestMembershipController:
                                  infoAboutUserController);
                        },
                      );
                    } else {
                      return Center(
...
Widget _clubItem(
    ...
    required TextEditingController requestMembershipController}) {
  bool alreadyJoinedToClub = Constants.userID != null
      ? myData!.idForClubsMemberIn != null &&
          myData.idForClubsMemberIn!.contains(club.id.toString())
      : false;
  bool clubAvailableAndHaveNotJoinedYetAndHaveNotSendRequestBefore =
      Constants.userID != null
          ? club.isAvailable == true &&
              (myData!.idForClubsMemberIn == null ||
                  (myData.idForClubsMemberIn != null &&
                      myData.idForClubsMemberIn!.contains(club.id.toString()) ==
                          false)) &&
              (cubit.idForClubsIAskedToJoinAndWaitingResponse
                      .contains(club.id.toString()) ==
                  false)
          : false;
  bool clubNotAvailableAndHaveNotJoinedYet = Constants.userID != null
    ...
    child: GestureDetector(
      onTap: () {
        Navigator.push(
            context,
            MaterialPageRoute(
                builder: (context) => ViewClubDetailsScreen(club: club)));
      },
      child: Card(
          elevation: 0.2,
          child: Padding(
                      ...
                      Row(
                        ...
                        children: [
                            ...
                            _buttonItem(
                              ...
                              onTap: () {
                                Navigator.push(
                                    ...
                                    MaterialPageRoute(
                                        builder: (context) =>
                                            ViewClubDetailsScreen(club: club)));
                              },
                            ),
                          if (myData != null &&
                              myData.idForClubLead != null &&
                          ...
                          if (myData != null &&
                              myData.idForClubLead == null &&
                              Constants.userID !=
                                  null) // TODO: AS it will be shown only if a Visitor or Member Not Leader....
                            _buttonItem(
                                title: alreadyJoinedToClub
                                    ? "تم الالتحاق"
                                    : clubAvailableAndHaveNotJoinedYetAndHaveNotSendRequestBefore
                                        ? "إنضم إلينا"
                                        : clubNotAvailableAndHaveNotJoinedYet
                                            ? "غير متاح"
                                            : "تم طلب العضوية",
                                color: alreadyJoinedToClub
                                ...
                                onTap: () {
                                  if (clubAvailableAndHaveNotJoinedYetAndHaveNotSendRequestBefore) {
                                    askMembershipDialog(
                                        ...
                                        club: club,
                                        userEntity:
                                            LayoutCubit.getInstance(context)
                                                .userData!);
                                  } else if (alreadyJoinedToClub) {
                                    showToastMessage(
                                        context: context,
                                        message: "لقد تم الإنضمام للنادي  !!");
                                  } else if (clubNotAvailableAndHaveNotJoinedYet) {
                                    showToastMessage(
                                        context: context,
                                        message:
                                            " لقد تم إيقاف الإنضمام للنادي ",
                                        backgroundColor: AppColors.kRedColor);
                                  } else {
                                    showToastMessage(
                                        context: context,
                                        message:
                                            "تم طلب العضوية بالفعل وف انتظار موافقه الليدر");
                                  }
                                })
                        ],
                      )
