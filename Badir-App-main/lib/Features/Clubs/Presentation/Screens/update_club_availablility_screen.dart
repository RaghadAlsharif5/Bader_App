...
class UpdateClubAvailabilityScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    clubsCubit.selectedColleges =
        List<dynamic>.of(clubEntity.availableOnlyForThisCollege);
    return Directionality(
      textDirection: TextDirection.rtl,
      child: SafeArea(
          child: Scaffold(
        appBar: AppBar(
          title: const Text("التسجيل بالنادي"),
        ),
        body: Padding(
            ...
            child: BlocConsumer<ClubsCubit, ClubsStates>(
              ...
              return Column(
                ...
                children: [
                  ...
                  Text(
                    "السماح بلإنضمام لطلاب الكليات الآتية",
                    style: TextStyle(
                  ...
                  Expanded(
                    ...
                    children: Constants.colleges
                        .map((option) => CheckboxListTile(
                              title: Text(option),
                              value:
                                  clubsCubit.selectedColleges.contains(option),
                              onChanged: (status) {
                                clubsCubit.addOrRemoveOptionToSelectedColleges(
                                    status: status!, college: option);
                              },
                            ))
                        .toList(),
                  )),
                    ...
                    child: MaterialButton(
                        ...
                        onPressed: () {
                          if (clubsCubit.selectedColleges.isNotEmpty &&
                              ((clubEntity.isAvailable !=
                                      clubsCubit.clubAvailabilityStatus) ||
                                  (clubEntity
                                          .availableOnlyForThisCollege.length !=
                                      clubsCubit.selectedColleges.length) ||
                                  (clubEntity
                                          .availableOnlyForThisCollege.last !=
                                      clubsCubit.selectedColleges.last))) {
                            clubsCubit.updateClubAvailability(
                                userEntity: userEntity,
                                clubID: clubEntity.id.toString(),
                                isAvailable: clubsCubit.clubAvailabilityStatus,
                                availableOnlyForThisCollege:
                                    clubsCubit.selectedColleges);
                          } else if (clubEntity.isAvailable ==
                                  clubsCubit.clubAvailabilityStatus &&
                              clubEntity.availableOnlyForThisCollege.length ==
                                  clubsCubit.selectedColleges.length &&
                              clubEntity.availableOnlyForThisCollege.first ==
                                  clubsCubit.selectedColleges.first &&
                              clubEntity.availableOnlyForThisCollege.last ==
                                  clubsCubit.selectedColleges.last) {
                            showToastMessage(
                                context: context,
                                message:
                                    'لم يحدث أي تعديل على البيانات القديمة !!',
