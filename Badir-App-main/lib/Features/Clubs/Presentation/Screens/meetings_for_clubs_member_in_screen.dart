...
class MeetingsForClubsMemberInScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    List idForClubsMemberID =
        LayoutCubit.getInstance(context).userData!.idForClubsMemberIn!;
    final ClubsCubit clubsCubit = ClubsCubit.getInstance(context)
      ..getMeetingRelatedToClubIMemberIn(
          idForClubsMemberIn: idForClubsMemberID);
    return Directionality(
      textDirection: TextDirection.rtl,
      child: SafeArea(
        child: Scaffold(
          appBar: AppBar(
            title: const Text("الإجتماعات"),
          ),
          body: BlocConsumer<ClubsCubit, ClubsStates>(
              buildWhen: (pastState, currentState) =>
                  currentState is GetMeetingRelatedToClubIMemberInSuccessState,
              listener: (context, state) {},
              builder: (context, state) {
                return Padding(
                  padding:
                      EdgeInsets.symmetric(vertical: 10.h, horizontal: 10.w),
                  child: state is GetMeetingRelatedToClubIMemberInLoadingState
                      ? const Center(child: CircularProgressIndicator())
                      : state is GetMeetingRelatedToClubIMemberInSuccessState &&
                          ...
                          ? ListView.separated(
                              ...
                              itemBuilder: (context, index) => _meetingItem(
                                  cubit: clubsCubit,
                                  context: context,
                                  meetingEntity: state.meetings[index]))
