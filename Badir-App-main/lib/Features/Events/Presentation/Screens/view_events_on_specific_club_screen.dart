...
class ViewEventsOnSpecificClubScreen extends StatelessWidget {
  ...
  const ViewEventsOnSpecificClubScreen(
  ...
  Widget build(BuildContext context) {
    ...
    final EventsCubit eventCubit = EventsCubit.getInstance(context)
      ..getEventsOnSpecificClub(
          clubID: clubID, idForClubILead: userEntity.idForClubLead);
              ...
              body: BlocConsumer<EventsCubit, EventsStates>(
                  ...
                  builder: (context, state) {
                    return state is GetEventsForSpecificClubLoadingState
                        ...
                        : state is GetEventsForSpecificClubSuccessState &&
                                state.eventsOnSpecificClub.isNotEmpty
                            ? _displayEvents(
                                eventsCubit: eventCubit,
                                layoutCubit: layoutCubit,
                                userEntity: userEntity,
                                context: context,
                                events: state.eventsOnSpecificClub)
                            : Center(
                                child: Text(
                                "لا توجد فعاليات بهذا النادي",
                                style: TextStyle(
              ...
              Navigator.push(
                  ...
                  MaterialPageRoute(
                      builder: (context) => EventDetailsScreen(
                          event: events[index], eventExpired: eventFinished)));
            },
            child: Card(
              child: Padding(
                        ...
                        onPressed: () {
                          if (userEntity.idForClubLead != null) {
                            ...
                            showToastMessage(
                                context: context,
                                message:
                                    'هذه الفعالية خاصة بأعضاء ${events[index].clubName} فقط',
