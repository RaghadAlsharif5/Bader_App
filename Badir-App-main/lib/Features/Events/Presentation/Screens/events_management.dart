...
class EventsManagementScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (eventCubit.ownEvents.isEmpty)
      eventCubit.getPastAndNewAndMyEvents(idForClubILead: idForClubILead);
        ...
        child: Scaffold(
          ...
          body: BlocConsumer<EventsCubit, EventsStates>(
              buildWhen: (past, currentState) =>
                  currentState is EventsClassifiedSuccessState,
              ...
              builder: (context, state) {
                return Padding(
                  ...
                  child: state is EventsClassifiedLoadingState
                      ? const Center(child: CircularProgressIndicator())
                      : eventCubit.ownEvents.isNotEmpty
                          ? ListView.separated(
                              physics: const BouncingScrollPhysics(),
                              itemCount: eventCubit.ownEvents
                              ...
                              itemBuilder: (context, index) => _eventItem(
                                  ...
                                  cubit: eventCubit,
                                  context: context,
                                  eventData: eventCubit.ownEvents[index]))
                          : Center(
                              child: Text(
                                "لا توجد فعاليات حتى الان",
                                style: TextStyle(
                                    fontSize: 15.sp,
    ...
    return GestureDetector(
      onTap: () {
        ...
        DateTime eventDate = Jiffy(
                "${eventData.endDate!.trim()} ${eventData.time!.trim()}",
                "MMMM dd, yyyy h:mm a")
            .dateTime;
        Navigator.push(
            context,
            MaterialPageRoute(
                builder: (context) => EventDetailsScreen(
                    event: eventData,
                    eventExpired: DateTime.now().isAfter(eventDate))));
      },
      child: Container(
                ...
                _buttonItem(
                  ...
                  onTap: () {
                    Navigator.push(
                        ...
                        MaterialPageRoute(
                            builder: (context) =>
                                UpdateEventScreen(eventEntity: eventData)));
                  },
                ),
                SizedBox(
                  width: 5.w,
                ...
                _buttonItem(
                  ...
                  onTap: () {
                    Navigator.push(
                        context,
                        MaterialPageRoute(
                            builder: (context) => ViewOpinionsAboutEventScreen(
                ...
                _buttonItem(
                    ...
                    onTap: () {
                      Navigator.push(
                          context,
                          MaterialPageRoute(
                              builder: (context) => ViewMembersOnAnEventScreen(
                                  eventID: eventData.id!.trim())));
                    }),
                SizedBox(
                  width: 5.w,
                ),
