...
class UpdateTaskScreen extends StatelessWidget {
  ...
  final TextEditingController _numOfPositionsController =
  ...
  Widget build(BuildContext context) {
    ...
    EventsCubit eventsCubit = EventsCubit.getInstance(context)
      ..getNamesOfEventsToUseInCreatingTask(idForClubILead: idForClubILead);
    ...
    eventsCubit.eventNameForTaskCreated =
        taskEntity.forPublicOrSpecificToAnEvent
            ? 'للعامة'
            : taskEntity.eventName;
          ...
          child: BlocConsumer<EventsCubit, EventsStates>(
            listener: (context, state) {
              ...
              if (state is UpdateTaskSuccessState) {
                ...
                showToastMessage(
                    context: context, message: "تم تعديل المهمة بنجاح");
              ...
              return ListView(
                children: [
                  ...
                  DefaultButton(
                      ...
                      onTap: () {
                        if (eventsCubit.eventNameForTaskCreated != null &&
                            _nameController.text.isNotEmpty &&
                            _descriptionController.text.isNotEmpty &&
                            _numOfHoursController.text.isNotEmpty &&
                            _numOfPositionsController.text.isNotEmpty) {
                          bool taskForPublicOrSpecificEvent = eventsCubit
                                  .namesForEventsICreated
                                  .contains(eventsCubit.eventNameForTaskCreated)
                              ? false
                              : true;
                          eventsCubit.updateTask(
                              taskID: taskEntity.id,
                              taskName: _nameController.text,
                              clubID: taskEntity.clubID,
                              hours:
                                  int.parse(_numOfHoursController.text.trim()),
                              idForClubILead: idForClubILead,
                              ownerID: Constants.userID ?? taskEntity.ownerID,
                              numOfPosition: int.parse(
                                  _numOfPositionsController.text.trim()),
                              numOfRegistered: taskEntity.numOfRegistered,
