...
class EventsCubit extends Cubit<EventsStates> {
  ...
  static EventsCubit getInstance(BuildContext context) =>
  ...
  List<EventEntity> ownEvents =
      []; // TODO: Mean that created by me | Related Club That I lead
  List<String> namesForEventsICreated =
      []; // TODO: Mean that created by me | Related Club That I lead  هستعملها في حاله انشاء تاسك مع DropDownButton
  ...
  Future<void> getEventsOnSpecificClub(
    ...
    emit(GetEventsForSpecificClubSuccessState(
        eventsOnSpecificClub: eventsOnSpecificClub));
  }
  ...
  Future<void> getMembersOnAnEvent({required String eventID}) async {
    ...
    final result =
        await sl<GetMembersOnAnEventUseCase>().execute(eventID: eventID);
    result.fold((serverFailure) {
      emit(FailedToGetMembersOnAnEventDataState(
  ...
  void updateEvent(
      ...
      required EventForPublicOrNot forPublic,
    ...
    EventModel eventModel = EventModel(
        ...
        clubName,
        clubID);
    final result = await sl<UpdateEventUseCase>()
        .execute(eventID: eventID, eventModel: eventModel);
    result.fold((serverFailure) {
  ...
  Future<void> getPastAndNewAndMyEvents({String? idForClubILead}) async {
    ...
    for (int i = 0; i < allEvents.length; i++) {
      DateTime eventDate = Jiffy(
              "${allEvents[i].endDate!.trim()} ${allEvents[i].time!.trim()}",
              "MMMM dd, yyyy h:mm a")
          .dateTime;
  ...
  void createEvent(
    ...
    if (imageUrl != null) {
      final result = await sl<CreateEventUseCase>().execute(
          forPublic: forPublic,
          name: name,
      ...
      FailedToCreateEventState(
          message: "حدثت مشكله اثناء رفع الصورة الرجاء المحاوله لاحقا");
    }
  }
  ...
  void deleteEvent(
      {required String eventID, required String idForClubILead}) async {
    emit(DeleteEventLoadingState());
    final result = await sl<DeleteEventUseCase>()
        .execute(eventID: eventID, clubID: idForClubILead);
    result.fold((serverFailure) {
  ...
  List<String> taskOptionsForCreateIt =
      []; // TODO: include names of eventsName that create by me + forPublic
  Future<void> getNamesOfEventsToUseInCreatingTask(
      {required String idForClubILead}) async {
    taskOptionsForCreateIt.clear();
    if (ownEvents.isEmpty)
      await getPastAndNewAndMyEvents(
          idForClubILead: idForClubILead); // TODO: عشان هستعمل ownEvents
    for (var element in ownEvents) {
      taskOptionsForCreateIt.add(element.name!);
    }
  ...
  Future<EventEntity> getEventDataThrowItsNameToUseItForCreatingTask(
    ...
    return ownEvents
        .firstWhere((eventItem) => eventItem.name!.trim() == eventName.trim());
  }
  ...
  void createTask(
    ...
    if (taskForPublicOrSpecificEvent == false)
      eventEntitySelectedForTask =
          await getEventDataThrowItsNameToUseItForCreatingTask(
              eventName: eventNameForTaskCreated!);
    final result = await sl<CreateTaskUseCase>().execute(
        ownerID: ownerID,
        taskName: taskName,
        clubID: idForClubILead,
        eventID: eventEntitySelectedForTask?.id,
        eventName: eventEntitySelectedForTask?.name,
        description: description,
        forPublicOrSpecificToAnEvent: taskForPublicOrSpecificEvent,
        hours: hours,
  ...
  void joinToEvent(
    ...
    final result = await sl<JoinToEventUseCase>()
        .execute(eventID: eventID, memberID: memberID);
    result.fold((serverFailure) {
      emit(FailedToJoinToEventState(message: serverFailure.errorMessage));
    }, (unit) async {
      await layoutCubit
          .getMyData(); // TODO: لأن حصل في تعديل في الداتا بتاعته وخصوصا في idForEventsJoined
  ...
  Future<void> getTasksCreatedByMe({required String idForClubILead}) async {
    ...
    for (var item in allTasksOnApp) {
      if (item.ownerID == Constants.userID ||
          (item.clubID.trim() == idForClubILead.trim())) {
  ...
  Future<void> getAvailableTasks({required UserEntity myData}) async {
    ...
    for (var item in allTasksOnApp) {
      if (item.forPublicOrSpecificToAnEvent ||
          (myData.idForClubsMemberIn != null &&
              myData.idForClubsMemberIn!.contains(item
                  .clubID))) // Public or related to club i'm member on it...
      {
        availableTasks.add(item);
      }
    }
    await getIDForTasksIAskedToAuthenticate(
        userID: myData.id!,
        idForClubsIMemberIn: myData
            .idForClubsMemberIn); // TODO: عشان بس لو بعت طلب اظهر للمستخد انه تم ارسال طلب عشان ميبعتش تاني
    emit(GetAvailableTasksSuccessState());
  }

  void deleteTask(
      {required String taskID, required String idForClubILead}) async {
  ...
  Future<void> getRequestForAuthenticateOnATask(
      {required String taskID}) async {
    emit(GetRequestForAuthenticateOnATaskLoadingState());
    final result = await sl<GedRequestForAuthenticateOnATaskUseCase>()
        .execute(taskID: taskID);
    result.fold((serverFailure) {
      emit(FailedToGetRequestForAuthenticateOnATaskState(
          message: serverFailure.errorMessage));
    }, (requestsData) async {
      emit(
          GetRequestForAuthenticateOnATaskSuccessState(requests: requestsData));
    });
  }

  // TODO: Will pass RequestsData to GedRequestForAuthenticateOnATaskSuccessState and display it on screen using state.....
  Future<void> acceptOrRejectAuthenticateRequestOnATask(
      {required String myID,
      required LayoutCubit layoutCubit,
      required String requestSenderName,
      required TaskEntity taskEntity,
      required String requestFirebaseFCMToken,
      required String requestSenderID,
      required bool respondStatus}) async {
    emit(AcceptOrRejectAuthenticateRequestOnATaskLoadingState());
    final result = await sl<AcceptOrRejectAuthenticateRequestOnATaskUseCase>()
        .execute(
            myID: myID,
            ...
            requestFirebaseFCMToken: requestFirebaseFCMToken,
            respondStatus: respondStatus);
    result.fold((serverFailure) {
      emit(FailedToAcceptOrRejectAuthenticateRequestOnATaskState(
          message: serverFailure.errorMessage));
    }, (unit) async {
  ...
  Future<void> sendOpinionAboutEvent(
      {required String eventID,
      required OpinionAboutEventModel opinionModel,
      required String senderID}) async {
    emit(SendOpinionAboutEventLoadingState());
    final result = await sl<SendOpinionAboutEventUseCase>().execute(
        eventID: eventID, opinionModel: opinionModel, senderID: senderID);
    result.fold((serverFailure) {
      emit(FailedToSendOpinionAboutEventState(
          message: serverFailure.errorMessage));
    }, (unit) async {
  ...
  Future<void> getOpinionsAboutEvent({required String eventID}) async {
    ...
    final result =
        await sl<GetOpinionsAboutEventUseCase>().execute(eventID: eventID);
    result.fold((serverFailure) {
      emit(FailedToGetOpinionsAboutEventState(
          message: serverFailure.errorMessage));
    }, (opinionsData) async {
  ...
  // TODO: User or Member ....
  void requestToAuthenticateOnATask(
      {required UserEntity myData,
      required String taskID,
      required String senderFirebaseFCMToken,
      required String senderID,
      required String senderName}) async {
    // emit(RequestAuthenticateOnATaskLoadingState());
    final result = await sl<RequestAuthenticationOnATaskUseCase>().execute(
        senderFirebaseFCMToken: senderFirebaseFCMToken,
        taskID: taskID,
        senderID: senderID,
        senderName: senderName);
    result.fold((serverFailure) {
      emit(FailedToRequestAuthenticateOnATaskState(
          message: serverFailure.errorMessage));
    }, (unit) async {
  ...
  Future<void> getIDForTasksIAskedToAuthenticate(
      {List? idForClubsIMemberIn, required String userID}) async {
    final result =
        await sl<GetIDForTasksThatIAskedForAuthenticationBeforeUseCase>()
            .execute(userID: userID, idForClubIMemberIn: idForClubsIMemberIn);
    result.fold((serverFailure) {
      emit(FailedToGetIDForTasksIAskedToAuthenticateState(
          message: serverFailure.errorMessage));
    }, (tasksID) {
      idForTasksThatIAskedToAuthenticateBefore = tasksID;
      debugPrint(
          "tasksID length is : ${tasksID.length}, idForTasksThatIAskedToAuthenticateBefore is $idForTasksThatIAskedToAuthenticateBefore");
  ...
  void updateTask(
    ...
    TaskModel taskModel = TaskModel(
        ...
        eventEntitySelectedForTask?.name,
        clubID,
        eventEntitySelectedForTask?.id);
    final result = await sl<UpdateTaskUseCase>()
        .execute(taskID: taskID.toString(), taskModel: taskModel);
    result.fold((serverFailure) {
