...
class RemoteEventsDataSource {
  ...
  Future<int> getNumOfEventsOnSpecificClub({required String clubID}) async {
    ...
    await FirebaseFirestore.instance
        ...
        .then((value) {
      eventsNum =
          value.data() != null ? value.data()!['currentEventsNum'] ?? 0 : 0;
    });
    return eventsNum;
  }
  ...
  Future<Unit> createEvent(
    ...
    try {
      ...
      QuerySnapshot querySnapshot = await FirebaseFirestore.instance
          ...
          .get();
      int newEventID = querySnapshot.docs.isNotEmpty
          ? int.parse(querySnapshot.docs.last.id)
          : 0;
      ++newEventID;
      // TODO: Update CurrentEventsNum on this club
      int currentEventsNumOnThisClub =
          await getNumOfEventsOnSpecificClub(clubID: clubID);
      await FirebaseFirestore.instance
          .collection(Constants.kClubsCollectionName)
          .doc(clubID)
          .update({"currentEventsNum": ++currentEventsNumOnThisClub});
  ...
  Future<Unit> createTask(
      ...
      String? eventID,
      String? eventName,
      required bool forPublicOrSpecificToAnEvent,
      required int hours,
      required int numOfPosition}) async {
    try {
      // TODO: Get Last ID For Last Task Created to increase it by one
      QuerySnapshot querySnapshot = await FirebaseFirestore.instance
          .collection(Constants.kTasksCollectionName)
          .get();
      int newTaskID = querySnapshot.docs.isNotEmpty
          ? int.parse(querySnapshot.docs.last.id)
          : 0;
      ++newTaskID;
      TaskModel taskModel = TaskModel(
          ...
          numOfPosition,
          0,
          forPublicOrSpecificToAnEvent,
          eventName,
          clubID,
          eventID);
      await FirebaseFirestore.instance
          .collection(Constants.kTasksCollectionName)
          .doc(newTaskID.toString().trim())
  ...
  Future<Unit> updateEvent(
      {required String eventID, required EventModel eventModel}) async {
    try {
      await FirebaseFirestore.instance
          .collection(Constants.kEventsCollectionName)
          .doc(eventID)
  ...
  Future<Unit> joinToEvent(
    ...
    try {
      ...
      idForEventsJoined
          .add(eventID); // TODO: add id for event in member Data on Firestore
      MemberModel member = MemberModel(
          memberName: userEntity.name!,
          memberID: memberID,
      ...
      await FirebaseFirestore.instance
          .collection(Constants.kUsersCollectionName)
          .doc(memberID)
          .update({'idForEventsJoined': idForEventsJoined});
      return unit;
    } on FirebaseException catch (e) {
      throw ServerException(exceptionMessage: e.code);
    }
  ...
  Future<List<MemberModel>> getMembersForAnEvent(
    ...
    try {
      ...
      await FirebaseFirestore.instance
          ...
          .then((value) {
        debugPrint(
            "Start getting data , documents length is ${value.docs.length}......");
        members = List<MemberModel>.of(
            value.docs.map((e) => MemberModel.fromJson(json: e.data())));
      });
      return members;
    } on FirebaseException catch (e) {
      throw ServerException(exceptionMessage: e.code);
  ...
  Future<Unit> updateTask(
      {required String taskID, required TaskModel taskModel}) async {
    try {
      await FirebaseFirestore.instance
          .collection(Constants.kTasksCollectionName)
          .doc(taskID)
  ...
  Future<Unit> requestToAuthenticateOnATask(
      {required String taskID,
      required String senderID,
      String? senderFirebaseFCMToken,
      required String senderName}) async {
    try {
      RequestAuthenticationOnATaskModel requestModel =
          RequestAuthenticationOnATaskModel(
              senderID, senderFirebaseFCMToken!, senderName);
      await FirebaseFirestore.instance
          .collection(Constants.kTasksCollectionName)
          .doc(taskID)
          .collection(Constants.kTaskAuthenticationRequestsCollectionName)
          .doc(senderID)
  ...
  Future<Unit> sendOpinionAboutEvent(
    ...
    try {
      await FirebaseFirestore.instance
          .collection(Constants.kEventsCollectionName)
          .doc(eventID)
          .collection(Constants.kOpinionsAboutTaskCollectionName)
          .doc(senderID)
          .set(opinionModel.toJson());
      return unit;
    } on FirebaseException catch (e) {
      throw ServerException(exceptionMessage: e.message!);
    }
  ...
  Future<List<OpinionAboutEventModel>> getOpinionsAboutEvent(
      {required String eventID}) async {
    try {
  ...
  // TODO: Leader ....
  Future<List<RequestAuthenticationOnATaskModel>>
      gedRequestForAuthenticateOnATask({required String taskID}) async {
    try {
      ...
      await FirebaseFirestore.instance
          ...
          .get()
          .then((value) {
        requests = List<RequestAuthenticationOnATaskModel>.of(value.docs
            .map((e) => RequestAuthenticationOnATaskModel.fromJson(json: e)));
      });
      return requests;
    } on FirebaseException catch (e) {
      throw ServerException(exceptionMessage: e.message!);
  ...
  Future<Set> getIDForTasksIAskedToAuthenticate(
    ...
    await FirebaseFirestore.instance
      ...
      if (value.docs.isNotEmpty) {
        for (var item in value.docs) {
          if (item.data()['forPublicOrSpecificToAnEvent'] == true ||
              (idForClubsIMemberIn != null &&
                  idForClubsIMemberIn
                      .contains(item.data()['clubID'].toString()))) {
  ...
  Future<int> getTotalVolunteerHoursOnApp() async {
    int hours = 0;
    await FirebaseFirestore.instance
        .collection(Constants.kTotalVolunteerHoursThrowAppCollectionName)
        .doc('Number')
        .get()
        .then((value) {
  ...
  Future<int> getTotalVolunteerHoursForSpecificClub(
  ...
  Future<Unit> acceptOrRejectAuthenticateRequestOnATask(
      ...
      String? requestFirebaseFCMToken,
    ...
    try {
      ...
      await layoutCubit.sendNotification(
          ...
          notifyContent: respondStatus
              ? "لقد تم قبول طلبك للمصادقة في مهمه ${taskEntity.name}"
              : "لقد تم رفض طلبك للمصادقة في مهمه ${taskEntity.name}",
          notifyType: respondStatus
              ? NotificationType.acceptYourRequestToAuthenticateOnATask
              : NotificationType.rejectYourRequestToAuthenticateOnATask);
      if (respondStatus) // TODO: Request Accepted
      {
        ...
        List? idForTasksAuthenticate =
            requestSenderUserModel.idForTasksAuthenticate ?? [];
        ...
        volunteerHoursForUser +=
            taskEntity.hours; // TODO: هتضيف عدد ساعات التاسك للشخص ده
        ...
        await FirebaseFirestore.instance
            .collection(Constants.kTotalVolunteerHoursThrowAppCollectionName)
            .doc('Number')
            .set({'total': totalVolunteerHours});
        // TODO: Update volunteer num on The Club that have this Task .....
        int volunteerHoursForClub = await getTotalVolunteerHoursForSpecificClub(
            clubID: taskEntity.clubID);
        volunteerHoursForClub += taskEntity.hours;
        await FirebaseFirestore.instance
            .collection(Constants.kClubsCollectionName)
            .doc(taskEntity.clubID)
            .update({
