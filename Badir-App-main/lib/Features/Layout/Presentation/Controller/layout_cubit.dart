...
class LayoutCubit extends Cubit<LayoutStates> {
  ...
  Future<void> uploadReport(
    ...
    if (pdfUrl != null) {
      final result = await sl<UploadReportToAdminUseCase>().execute(
          clubName: clubName,
          senderID: senderID,
          clubID: clubID,
          pdfLink: pdfUrl,
  ...
  Future<void> logout({
    ...
    var result = await sl<LogOutUseCase>().execute(
        layoutCubit: layoutCubit,
        clubsCubit: clubsCubit,
        eventsCubit: eventsCubit);
    result.fold(
        (serverFailure) =>
            emit(FailedToLogOut(message: serverFailure.errorMessage)), (unit) {
  ...
  Future<void> getMyData({ClubsCubit? clubsCubit}) async {
    ...
    result.fold(
        (serverFailure) =>
            emit(FailedToGetUserDataState(message: serverFailure.errorMessage)),
        (user) async {
      userData = user;
  ...
  void updateMyData(
    ...
    bool dataUpdatedSuccess = await sl<UpdateMyDataUseCase>()
        .execute(name: name, college: college, gender: gender, phone: phone);
  ...
  Future<void> sendNotification(
    ...
    var result = await sl<SendNotificationUseCase>().execute(
    ...
    result.fold(
        (serverFailure) => emit(
            FailedSendNotificationState(message: serverFailure.errorMessage)),
        (unit) {
