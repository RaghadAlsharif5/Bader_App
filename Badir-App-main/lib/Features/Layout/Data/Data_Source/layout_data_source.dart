...
class LayoutRemoteDataSource {
  ...
  Future<void> sendNotification(
    ...
    try {
      NotifyModel notifyModel = NotifyModel(Constants.getTimeNow(),
          notifyType.name, false, notifyContent, clubID);
      if (receiverFirebaseToken != null || topicName != null)
        await notifyUserOrNumberOfUsersUsingFCMAPI(
            notifyTitle: notifyTitle,
            topicName: topicName,
            toSpecificUserOrNumOfUsers: toSpecificUserOrNumOfUsers,
            receiverFirebaseMessagingToken: receiverFirebaseToken,
            notifyType: notifyType,
            notifyBody: notifyContent);
      await FirebaseFirestore.instance
          .collection(Constants.kUsersCollectionName)
          .doc(receiverID)
          .collection(Constants.kNotificationsCollectionName)
  ...
  Future<Unit> uploadReport(
    ...
    try {
      QuerySnapshot querySnapshot = await FirebaseFirestore.instance
          ...
          .get();
      int newReportID = querySnapshot.docs.isNotEmpty
  ...
  Future<void> notifyUserOrNumberOfUsersUsingFCMAPI(
    ...
    await http.post(Constants.firebaseFCMAPIUri,
        headers: {
          'Content-Type': "application/json",
          'Authorization': Constants.serverKey
        },
        body: jsonEncode({
          "to": toSpecificUserOrNumOfUsers
              ? receiverFirebaseMessagingToken
              : "/topics/$topicName",
          "notification": {
            "title": notifyTitle,
            "body": notifyBody,
