...
class EventDetailsScreen extends StatelessWidget {
  ...
  EventDetailsScreen(
      {Key? key, required this.event, required this.eventExpired})
      : super(key: key);
  ...
  Widget build(BuildContext context) {
    ...
    UserEntity? userEntity = Constants.userID != null
        ? layoutCubit.userData!
        : null; // TODO: Can be null if user is a Visitor
    final EventsCubit eventsCubit = EventsCubit.getInstance(context);
    // TODO: three variables eventExpiredAndIHaveJoined, eventInDateAndIHaveJoined, eventInDateAndIHaveNotJoinedYetAndHavePermission check value of userID لان ماراح نستخدمهم الا اذا كان المستخدم سوى تسجيل دخول بالفعل
    bool eventExpiredAndIHaveJoined = Constants.userID != null
        ? Constants.eventExpiredAndIHaveJoined(
            event: event, eventExpired: eventExpired, userEntity: userEntity!)
        : false;
          ...
          body: Padding(
              ...
              child: ListView(
                ...
                children: [
                  ...
                  _textFieldItem(
                      controller: _nameController, title: 'اسم الفعالية'),
                  _textFieldItem(
                      controller: _descriptionController,
                      title: 'الوصف',
                      maxLines: 6),
                  _textFieldItem(
                      controller: _startDateController, title: 'تاريخ البدء'),
                  _textFieldItem(
                      controller: _endDateController, title: 'تاريخ الانتهاء'),
                  ...
                  Column(
                    ...
                    children: [
                      ...
                      Text(
                        event.forPublic == EventForPublicOrNot.private.name
                            ? 'خاصة بأعضاء ${event.clubName} فقط'
                            : 'للمستخدمين جميعا',
                        style: TextStyle(
                      ...
                      onTap: () {
                        if (eventExpiredAndIHaveJoined) {
                          Navigator.push(
                              ...
                              MaterialPageRoute(
                                  builder: (context) =>
                                      SendOpinionAboutEventScreen(
                                          eventName: event.name!,
                                          eventID: event.id!)));
                        } else if (eventInDateAndIHaveNotJoinedYetAndHavePermission) {
                          eventsCubit.joinToEvent(
                              eventID: event.id!,
                              layoutCubit: layoutCubit,
                              memberID: userEntity.id ?? Constants.userID!);
                        } else if (eventInDateAndIHaveJoined) {
                          showToastMessage(
                              context: context,
                              message: "لقد سبق لك التسجيل بالفعالية",
                              backgroundColor: AppColors.kRedColor);
                        }
                      },
                      title: eventExpiredAndIHaveJoined
                          ? "شاركنا برأيك"
                          : eventInDateAndIHaveJoined
                              ? "تم التسجيل"
                              : "سجل الآن",
