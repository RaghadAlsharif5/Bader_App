...
class EventItemOnHomeScreen extends StatelessWidget {
  ...
  EventItemOnHomeScreen(
      {Key? key,
      required this.context,
      required this.eventEntity,
      required this.layoutCubit,
      required this.eventsCubit,
      required this.eventExpired,
      this.myData})
      : super(key: key);

  @override
  Widget build(BuildContext context) {
    // TODO: بالنسبه للثلاث متغيرات هذه احنا مسوين if علي id حقت المستخدم عشان اعرف اذا كان visitor ولا لا
    ...
    bool eventInDateAndIHaveNotJoinedYetAndHavePermission =
        Constants.userID != null
            ? Constants.eventInDateAndIHaveNotJoinedYetAndHavePermission(
                ...
                event: eventEntity)
            : false;
    bool eventInDateAndIDoNotHavePermissionToJoin = Constants.userID != null
        ? Constants.eventInDateAndIDoNotHavePermissionToJoin(
            userEntity: myData!, eventExpired: eventExpired, event: eventEntity)
        : false;
    return InkWell(
      onTap: () {
        bool eventFinished = DateTime.now().isAfter(Jiffy(
                "${eventEntity.endDate!.trim()} ${eventEntity.time!.trim()}",
                "MMMM dd, yyyy h:mm a")
            .dateTime);
        Navigator.push(
            context,
            MaterialPageRoute(
                builder: (context) => EventDetailsScreen(
                    event: eventEntity, eventExpired: eventFinished)));
      },
      child: Container(
        width: 160.w,
          ...
          children: [
            ...
            Align(
              ...
              child: MaterialButton(
                  ...
                  if (Constants.userID == null) // TODO: He is a Visitor
                  {
                    ...
                    if (myData!.idForClubLead != null) {
                      Navigator.push(
                          ...
                          MaterialPageRoute(
                              builder: (context) => EventDetailsScreen(
                                  event: eventEntity, eventExpired: false)));
                    } else if (eventExpiredAndIHaveJoined) {
                      Navigator.push(
                          context,
                          MaterialPageRoute(
                              builder: (context) => SendOpinionAboutEventScreen(
                      ...
                      eventsCubit.joinToEvent(
                          ...
                          memberID: myData!.id ?? Constants.userID!);
                ...
                textColor: Constants.userID == null
