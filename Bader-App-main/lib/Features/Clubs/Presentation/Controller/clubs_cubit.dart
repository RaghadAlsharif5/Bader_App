...
class ClubsCubit extends Cubit<ClubsStates> {
  ...
  Future<void> getAllClubs({UserEntity? userEntity}) async {
    ...
    result.fold((serverFailure) {
      ...
      clubs = clubsData;
      if (userEntity != null && userEntity.idForClubLead == null)
        await getIDForClubsIAskedForMembership(
            userID: userEntity.id ?? Constants.userID!,
            idForClubsMemberID: userEntity.idForClubsMemberIn);
  ...
  Future<void> createMeeting(
    ...
    final result = await sl<CreateMeetingUseCase>().execute(
  ...
  Future<void> getMeetingRelatedToClubIMemberIn(
      {required List idForClubsMemberIn}) async {
    emit(GetMeetingRelatedToClubIMemberInLoadingState());
    final result = await sl<GetMeetingRelatedToClubIMemberInUseCase>()
        .execute(idForClubsMemberIn: idForClubsMemberIn);
    result.fold((serverFailure) {
      emit(GetMeetingRelatedToClubIMemberInWithFailureState(
          message: serverFailure.errorMessage));
    }, (meetingsData) {
      emit(
          GetMeetingRelatedToClubIMemberInSuccessState(meetings: meetingsData));
    });
  }
  ...
  Future<void> getMeetingsCreatedByMe({required String clubID}) async {
    ...
    final result =
        await sl<GetMeetingsCreatedByMeUseCase>().execute(clubID: clubID);
    result.fold((serverFailure) {
      emit(GetMeetingCreatedByWithFailureState(
          message: serverFailure.errorMessage));
    }, (meetings) {
  ...
  Future<void> deleteMeeting(
    ...
    final result = await sl<DeleteMeetingUseCase>()
        .execute(clubID: clubID, meetingID: meetingID);
    result.fold((serverFailure) {
  ...
  Future<void> updateClubAvailability(
    ...
    final result = await sl<UpdateClubAvailabilityUseCase>().execute(
        clubID: clubID,
        isAvailable: isAvailable,
        availableOnlyForThisCollege: availableOnlyForThisCollege);
    result.fold((serverFailure) {
      emit(UpdateClubAvailabilityWithFailureState(
          message: serverFailure.errorMessage));
    }, (unit) async {
  ...
  Future<void> getMembersDataOnMyClub(
    ...
    result.fold((serverFailure) {
      emit(GetMembersOnMyClubDataWithFailureState(
          message: serverFailure.errorMessage));
    }, (membersID) {
      debugPrint("All Users on App num is : ${usersOnApp.length}");
      // هنا راح نسوي لوب على المستخدمين بالكامل ولو لقينا id حقت في set اللي جايه من ال result راح نضيفها للداتا تبع الأعضاء
      ...
      debugPrint(
          "Members on My Club Number is : ${membersDataOnMyClub.length}");
  ...
  void changeSearchAboutClubStatus({bool? value}) {
    searchEnabled = value == null
        ? !searchEnabled
        : value; // TODO: لان ف بعض الحالات نبغى نخليها false ف راح نرسل value وقتها
  ...
  // TODO: هذه عشان الصفحة حقت تحديد الكليات اللي مسموح لها بالانضمام للنادي
  List selectedColleges = [];
  void addOrRemoveOptionToSelectedColleges(
      {required bool status, required String college}) {
  ...
  Future<void> getCLubDataThatILead({required String clubID}) async {
    dataAboutClubYouLead =
        clubs.firstWhere((element) => element.id == int.parse(clubID.trim()));
  ...
  Future<String?> uploadClubImageToStorage() async {
    String? url;
    final result = await sl<UploadClubImageToStorageUseCase>()
        .execute(imageFile: clubImage!);
    await Future.value(result.fold((noNetworkFailure) {
  ...
  void updateClubData(
    ...
    final clubUpdateResult = await sl<UpdateClubUseCase>().execute(
        ...
        memberNum: memberNum,
        aboutClub: aboutClub,
        contactInfo: contactMeansModel);
    await Future.value(clubUpdateResult.fold((serverFailure) {
  ...
  Future<void> getIDForClubsIAskedForMembership(
      {List? idForClubsMemberID, required String userID}) async {
    final clubUpdateResult = await sl<GetIDForClubsIAskedForMembershipUseCase>()
        .execute(userID: userID, idForClubsMemberID: idForClubsMemberID);
    clubUpdateResult.fold((serverFailure) {
      emit(FailedToGetIDForClubsIAskedForMembershipState(
          message: serverFailure.errorMessage));
    }, (clubsID) {
  ...
  void acceptOrRejectMembershipRequest(
      {required LayoutCubit layoutCubit,
      String? receiverFirebaseMessagingToken,
      required String committeeNameForRequestSender,
      required String idForClubILead,
      required String requestSenderID,
      required String clubID,
      required bool respondStatus,
      required String clubName}) async {
    emit(AcceptOrRejectMembershipRequestLoadingState());
    final result = await sl<AcceptOrRejectMembershipRequestUseCase>().execute(
        committeeNameForRequestSender: committeeNameForRequestSender,
        clubID: clubID,
        requestSenderID: requestSenderID,
        respondStatus: respondStatus);
    result.fold((serverFailure) {
      emit(FailedToAcceptOrRejectMembershipRequestState(
          message: serverFailure.errorMessage));
    }, (unit) async {
      await layoutCubit.sendNotification(
          toSpecificUserOrNumOfUsers: true,
          notifyTitle: "طلب العضوية",
          receiverFirebaseToken: receiverFirebaseMessagingToken,
          receiverID: requestSenderID,
          clubID: clubID,
          notifyContent: respondStatus
              ? "لقد تم قبول طلب العضوية في $clubName"
              : "لقد تم رفض طلب العضوية في $clubName",
          notifyType: respondStatus
              ? NotificationType.acceptYourMembershipRequest
              : NotificationType.rejectYourMembershipRequest);
      await getMembersDataOnMyClub(
          layoutCubit: layoutCubit, idForClubILead: idForClubILead);
      emit(AcceptOrRejectMembershipRequestSuccessState());
    });
  }

  void removeMemberFromCLubILead(
      {required String idForClubILead,
      required String memberID,
      required String memberFirebaseMessagingToken,
      required LayoutCubit layoutCubit,
      required String clubID,
      required String clubName}) async {
    emit(RemoveMemberFromClubLoadingState());
    final result = await sl<RemoveMemberFromClubILeadUseCase>()
        .execute(memberID: memberID, clubID: idForClubILead);
    result.fold((serverFailure) {
      emit(RemoveMemberFromClubWithFailureState(
          message: serverFailure.errorMessage));
    }, (unit) async {
      await layoutCubit.sendNotification(
          toSpecificUserOrNumOfUsers: true,
  ...
  void getMembershipRequests({required String clubID}) async {
    ...
    final result =
        await sl<GetMembershipRequestsUseCase>().execute(clubID: clubID);
    result.fold((serverFailure) {
      emit(FailedToGetMembershipRequestsState(
          message: serverFailure.errorMessage));
    }, (requests) async {
